...
const evrprogpow = require('../../index').evrprogpow;

////////////////////////////////////////////////////////////////////////////////

describe('Test algorithm functionality', () => {

  // Evrprogpow Validation [1]
  test('Test implemented evrprogpow algorithm [1]', () => {
    ...
    expect(evrprogpow.apply(null, [header, nonce, 262523, mixhash, output])).toBe(true);
  });

  // Evrprogpow Validation [2]
  test('Test implemented evrprogpow algorithm [2]', () => {
    const header = Buffer.from('63543d3913fe56e6720c5e61e8d208d05582875822628f483279a3e8d9c9a8b3', 'hex');
    const mixhash = Buffer.from('89732e5ff8711c32558a308fc4b8ee77416038a70995670e3eb84cbdead2e337', 'hex');
    const nonce = Buffer.from('9b95eb33003ba288', 'hex');
    const output = Buffer.alloc(32);
    expect(evrprogpow.apply(null, [header, nonce, 262524, mixhash, output])).toBe(false);
  });

  // Evrprogpow Validation [3]
  test('Test implemented evrprogpow algorithm [3]', () => {
    ...
    const output = Buffer.alloc(32);
    expect(evrprogpow.apply(null, [header, nonce, 1973071, mixhash, output])).toBe(true);
  });

  // Evrprogpow Validation [4]
  test('Test implemented evrprogpow algorithm [4]', () => {
    const header = Buffer.from('99ba4af95948377c47b1a6befc3a337f7b033ef6031a0c6a5ad3e727219653bd', 'hex');
    ...
    const output = Buffer.alloc(32);
    expect(evrprogpow.apply(null, [header, nonce, 1973071, mixhash, output])).toBe(false);
  });
});
