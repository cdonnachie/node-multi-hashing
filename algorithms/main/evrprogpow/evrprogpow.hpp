...
{
constexpr auto revision = EVRPROGPOW_REVISION;

static constexpr int epoch_length = EVRPROGPOW_EPOCH_LENGTH;
static constexpr int light_cache_item_size = EVRPROGPOW_LIGHT_CACHE_ITEM_SIZE;
static constexpr int full_dataset_item_size = EVRPROGPOW_FULL_DATASET_ITEM_SIZE;
static constexpr int num_dataset_accesses = EVRPROGPOW_NUM_DATASET_ACCESSES;

using epoch_context = evrprogpow_epoch_context;
using epoch_context_full = evrprogpow_epoch_context_full;

using result = evrprogpow_result;
...
/// Alias for evrprogpow_calculate_light_cache_num_items().
static constexpr auto calculate_light_cache_num_items = evrprogpow_calculate_light_cache_num_items;

/// Alias for evrprogpow_calculate_full_dataset_num_items().
static constexpr auto calculate_full_dataset_num_items = evrprogpow_calculate_full_dataset_num_items;

/// Alias for evrprogpow_calculate_epoch_seed().
static constexpr auto calculate_epoch_seed = evrprogpow_calculate_epoch_seed;
...
using epoch_context_ptr = std::unique_ptr<epoch_context, decltype(&evrprogpow_destroy_epoch_context)>;

using epoch_context_full_ptr =
    std::unique_ptr<epoch_context_full, decltype(&evrprogpow_destroy_epoch_context_full)>;

/// Creates Ethash epoch context.
///
/// This is a wrapper for evrprogpow_create_epoch_number C function that returns
...
{
    return {evrprogpow_create_epoch_context(epoch_number), evrprogpow_destroy_epoch_context};
}

inline epoch_context_full_ptr create_epoch_context_full(int epoch_number) noexcept
{
    return {evrprogpow_create_epoch_context_full(epoch_number), evrprogpow_destroy_epoch_context_full};
}
...
{
    return evrprogpow_hash(&context, &header_hash, nonce);
}
...
{
    return evrprogpow_verify_final_hash(&header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return evrprogpow_verify(&context, &header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return *evrprogpow_get_global_epoch_context(epoch_number);
}

/// Get global shared epoch context with full dataset initialized.
inline const epoch_context_full& get_global_epoch_context_full(int epoch_number) noexcept
{
    return *evrprogpow_get_global_epoch_context_full(epoch_number);
}
}  // namespace evrprogpow_main
